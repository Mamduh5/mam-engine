class_name DefensiveActionFixture
extends Node
...
func run_scenario(scenario: Dictionary) -> Dictionary:
	...
	return {
		...
		"invulnerabilityStartStep": floori(float(profile.invulnerabilityStartSeconds) / delta + EPSILON) + 1,
		"invulnerabilityEndStep": max(1, ceili(float(profile.invulnerabilityEndSeconds) / delta - EPSILON)),
