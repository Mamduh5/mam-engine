...
test("packed production consumer runs without npm or engine runtime fixtures", { skip: godot === undefined }, async (context) => {
  ...
  run(bin, ["project", "init", consumer, "--json"], root); run(bin, ["movement", "create", "movement/player.json", "--json"], consumer); run(bin, ["camera", "create", "camera/player.json", "--json"], consumer);
  ...
  for (const cameraAddonFile of ["mam_camera_bundle_loader.gd", "mam_camera_core.gd", "mam_camera_runtime.gd"]) await readFile(path.join(consumer, "addons", "mam_engine", "runtime", cameraAddonFile));
  ...
  assert.deepEqual(sync1.changedFiles, ["mam_generated/mam_camera_runtime_bundle.json", "mam_generated/mam_runtime_bundle.json"]); assert.deepEqual(sync2.changedFiles, []); assert.equal(check.status, "passed");
  ...
  assert.equal(valid.data.cameraLoad, "passed"); assert.equal(valid.data.cameraMissingBundleCode, "MAM_BUNDLE_MISSING"); assert.equal(valid.data.cameraInvalidBundleCode, "MAM_BUNDLE_INTEGRITY_MISMATCH"); assert.equal(valid.data.cameraBind, "passed"); assert.equal(valid.data.cameraIncompleteBind, "MAM_CAMERA_BIND_FOLLOW_TARGET_INVALID"); assert.ok(Number(valid.data.cameraOrbitYawDelta) > 45); assert.equal(valid.data.cameraPitchClamped, true); assert.ok(Number(valid.data.cameraFollowRigDistance) > 2.5); assert.equal(valid.data.cameraManualOrbitActive, true); assert.equal(valid.data.cameraRecenterDelayed, true); assert.equal(valid.data.cameraRecenterObserved, true); assert.ok(Number(valid.data.cameraRecenterErrorAfter) < Number(valid.data.cameraRecenterErrorBefore)); assert.equal(valid.data.cameraLensApplied, true); assert.ok(Math.abs(Number(valid.data.cameraForwardMagnitude) - 1) < 0.001); assert.ok(Math.abs(Number(valid.data.cameraRightMagnitude) - 1) < 0.001); assert.ok(Math.abs(Number(valid.data.cameraBasisDot)) < 0.001); assert.equal(valid.data.cameraUnbind, "passed");
