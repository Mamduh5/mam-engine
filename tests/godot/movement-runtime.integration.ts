...
test("runtime check completes a real cold-cache readiness smoke run", async (context) => {
  if (!await requireGodot(context)) return;
  await rm(path.join(projectRoot(), "runtime", "godot", ".godot"), { recursive: true, force: true });
  const result = await checkRuntime(projectRoot());
