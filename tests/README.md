...
Camera Editor Phase 2A.1 adds camera schema/semantic validation, math, fixed-step simulations, CLI coverage, safe edits, and kind-aware snapshots. Phase 2A.2 adds focused camera runtime protocol, comparison, and application/process tests plus real Godot cold-cache, orbit, pitch, recenter variants, follow, collision variants, basis, lens, clean-exit, cleanup, and changed-file evidence. Use `npm run test:camera-runtime` for the focused Node tier and `npm run test:godot:camera` for the camera integration file. `npm run test:godot` remains the complete movement-and-camera Godot regression tier used by CI.

Targeting Phase 2B.1 adds Node-only schema/validation, acquisition/scoring, deterministic tie, retention/loss/reacquisition, switching/cooldown, CLI, transaction/recovery/locking, and three-kind snapshot isolation tests. It adds no Godot test or runtime behavior.
