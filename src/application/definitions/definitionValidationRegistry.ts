...
export type SupportedDefinition = MovementProfile | CameraProfile | TargetingProfile | DefensiveActionProfile;
...
export function validateDefinition(value: unknown): DefinitionValidationResult {
  ...
  return {
    ...
    errors: [{ code: ErrorCodes.DefinitionKindUnsupported, path: "kind", message: "Definition kind must be movement-profile, camera-profile, targeting-profile, or defensive-action-profile", actual: recordKind(value), expected: ["movement-profile", "camera-profile", "targeting-profile", "defensive-action-profile"] }]
  };
}
