...
import type { StaminaActionProfile, StaminaActionSimulation, StaminaProfile } from "../../domain/stamina/staminaTypes";
import { RUNTIME_RUN_COMMAND, RUNTIME_SCHEMA_VERSION, STAMINA_FIXTURE_ID, type RuntimeResponse, type StaminaRuntimeRequest } from "../../domain/runtime/runtimeProtocol";
...
export interface RunStaminaFixtureOptions extends ProcessRunnerOptions { godot?: string; keepSession?: boolean; timeoutMs?: number }
export interface StaminaRuntimeFixtureExecution { executable: GodotExecutable; request: StaminaRuntimeRequest; readiness: RuntimeResponse; response: RuntimeResponse; process: Awaited<ReturnType<typeof runGodotProcess>>; simulation: StaminaActionSimulation; session: { retained: boolean; path: string | null }; runtimeSession: RuntimeSession; internalArtifacts: string[] }

export async function runStaminaFixture(workspaceRoot: string, stamina: StaminaProfile, action: StaminaActionProfile, options: RunStaminaFixtureOptions = {}): Promise<StaminaRuntimeFixtureExecution> {
  ...
  const correlationId = randomUUID(); const simulation = simulateStaminaAction(stamina, action);
  const request: StaminaRuntimeRequest = { schemaVersion: RUNTIME_SCHEMA_VERSION, commandId: RUNTIME_RUN_COMMAND, fixtureId: STAMINA_FIXTURE_ID, correlationId, requestedAt: new Date().toISOString(), timeoutMs: Math.min(Math.max(options.timeoutMs ?? 10_000, 1), 60_000), payload: { staminaDefinitionKind: "stamina-profile", staminaDefinitionSchemaVersion: 1, staminaProfile: stamina, actionDefinitionKind: action.kind, actionDefinitionSchemaVersion: 1, actionProfile: action, scenario: { id: "action-cost", durationSeconds: FIXED_DELTA_SECONDS, fixedDeltaSeconds: FIXED_DELTA_SECONDS } } };
  ...
  try {
    ...
    const ready = validateRuntimeResponse(readyValue, { correlationId, fixtureId: STAMINA_FIXTURE_ID, commandId: "runtime.fixture.ready", status: "ready" }); if (!ready.valid || !ready.value) throw responseValidationError(ready.errors, "readiness", session);
    ...
    const response = validateRuntimeResponse(responseValue, { correlationId, fixtureId: STAMINA_FIXTURE_ID, commandId: RUNTIME_RUN_COMMAND, status: ["ok", "rejected", "failed"], scenarioId: "action-cost" }); if (!response.valid || !response.value) throw responseValidationError(response.errors, "response", session); if (response.value.status !== "ok") throw new RuntimeFixtureError(ErrorCodes.RuntimeExecutionFailed, `Godot runtime returned ${response.value.status}`, session, { validationErrors: response.value.validationErrors, runtimeErrors: response.value.runtimeErrors });
