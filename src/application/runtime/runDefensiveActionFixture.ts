...
import { DEFENSIVE_ACTION_FIXED_DELTA_SECONDS, simulateDefensiveAction } from "../../domain/defensiveAction/defensiveActionSimulation";
import type { DefensiveActionProfile, DefensiveActionSimulation } from "../../domain/defensiveAction/defensiveActionTypes";
import { DEFENSIVE_ACTION_FIXTURE_ID, RUNTIME_RUN_COMMAND, RUNTIME_SCHEMA_VERSION, type DefensiveActionRuntimeRequest, type RuntimeResponse } from "../../domain/runtime/runtimeProtocol";
...
export interface RunDefensiveActionFixtureOptions extends ProcessRunnerOptions { godot?: string; keepSession?: boolean; timeoutMs?: number }
export interface DefensiveActionRuntimeFixtureExecution { executable: GodotExecutable; request: DefensiveActionRuntimeRequest; readiness: RuntimeResponse; response: RuntimeResponse; process: Awaited<ReturnType<typeof runGodotProcess>>; simulation: DefensiveActionSimulation; session: { retained: boolean; path: string | null }; runtimeSession: RuntimeSession; internalArtifacts: string[] }

export async function runDefensiveActionFixture(workspaceRoot: string, profile: DefensiveActionProfile, fixedDeltaSeconds = DEFENSIVE_ACTION_FIXED_DELTA_SECONDS, options: RunDefensiveActionFixtureOptions = {}): Promise<DefensiveActionRuntimeFixtureExecution> {
  ...
  const correlationId = randomUUID(); const simulation = simulateDefensiveAction(profile, fixedDeltaSeconds);
  const request: DefensiveActionRuntimeRequest = { schemaVersion: RUNTIME_SCHEMA_VERSION, commandId: RUNTIME_RUN_COMMAND, fixtureId: DEFENSIVE_ACTION_FIXTURE_ID, correlationId, requestedAt: new Date().toISOString(), timeoutMs: Math.min(Math.max(options.timeoutMs ?? 10_000, 1), 60_000), payload: { definitionKind: "defensive-action-profile", definitionSchemaVersion: 1, profile, scenario: { id: "default", durationSeconds: profile.durationSeconds + profile.cooldownSeconds, fixedDeltaSeconds } } };
  ...
  try {
    ...
    const ready = validateRuntimeResponse(readyValue, { correlationId, fixtureId: DEFENSIVE_ACTION_FIXTURE_ID, commandId: "runtime.fixture.ready", status: "ready" }); if (!ready.valid || !ready.value) throw responseValidationError(ready.errors, "readiness", session);
    ...
    const response = validateRuntimeResponse(responseValue, { correlationId, fixtureId: DEFENSIVE_ACTION_FIXTURE_ID, commandId: RUNTIME_RUN_COMMAND, status: ["ok", "rejected", "failed"], scenarioId: "default" }); if (!response.valid || !response.value) throw responseValidationError(response.errors, "response", session); if (response.value.status !== "ok") throw new RuntimeFixtureError(ErrorCodes.RuntimeExecutionFailed, `Godot runtime returned ${response.value.status}`, session, { validationErrors: response.value.validationErrors, runtimeErrors: response.value.runtimeErrors });
