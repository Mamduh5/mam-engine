...
import { COMBAT_FIXTURE_ID, RUNTIME_RUN_COMMAND, RUNTIME_SCHEMA_VERSION, type CombatRuntimeRequest, type RuntimeResponse } from "../../domain/runtime/runtimeProtocol";
...
export async function runCombatFixture(workspaceRoot: string, health: HealthProfile, action: OffensiveActionProfile, options: RunCombatFixtureOptions = {}): Promise<CombatRuntimeFixtureExecution> {
  ...
  try {
    ...
    const ready = validateRuntimeResponse(readyValue, { correlationId, fixtureId: COMBAT_FIXTURE_ID, commandId: "runtime.fixture.ready", status: "ready" }); if (!ready.valid || !ready.value) throw responseValidationError(ready.errors, "readiness", session);
    ...
    const response = validateRuntimeResponse(responseValue, { correlationId, fixtureId: COMBAT_FIXTURE_ID, commandId: RUNTIME_RUN_COMMAND, status: ["ok", "rejected", "failed"], scenarioId: "default" }); if (!response.valid || !response.value) throw responseValidationError(response.errors, "response", session); if (response.value.status !== "ok") throw new RuntimeFixtureError(ErrorCodes.RuntimeExecutionFailed, `Godot runtime returned ${response.value.status}`, session, { validationErrors: response.value.validationErrors, runtimeErrors: response.value.runtimeErrors });
