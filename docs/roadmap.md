...
## Phase 10 — Visual editor client (Phases 10A and 10B complete)

Phase 10A provides the loopback-only local editor foundation and read-only definition exploration. Phase 10B completes one movement-profile workflow: primitive single-property editing, dry-run preview, revision-conflict protection, transactional save with the existing snapshot system, and undo through existing rollback behavior. Other definition kinds remain read-only; simulation controls and broader visual authoring workflows remain pending. **Phase 10 exit condition:** the visual editor can complete supported authoring workflows without a second domain model, while CLI compatibility and headless automation remain intact.
