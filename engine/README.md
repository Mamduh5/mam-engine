...
Infrastructure under [`src/infrastructure/`](../src/infrastructure/) supplies Ajv schema validation, safe JSON I/O, workspace state capture, changed-file audits, and snapshot storage. Movement, camera, and targeting domain services do not depend on terminal output, a GUI, Godot, fixture scenes, or source-patching heuristics.

Targeting authored operations live under `src/domain/targeting/` and `src/application/targeting/`. The profile owns policy; ephemeral candidates and lock state belong to deterministic simulations. Phase 2B.2 plan/simulation and comparison/orchestration live in shared runtime seams; Godot receives normalized values and no canonical paths.
