...
Targeting Phase 2B.1 lives under `src/domain/targeting/` and `src/application/targeting/`. The profile owns policy; ephemeral candidates and lock state belong to deterministic simulations. Targeting set reuses the shared lock, transaction, exact recovery, audit, snapshot, and rollback infrastructure.
